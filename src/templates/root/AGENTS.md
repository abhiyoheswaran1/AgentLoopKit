...
- Run `agentloop loop create`, `agentloop loop tick`, `agentloop loop run`, `agentloop loop scorecard`, `agentloop loop status`, and `agentloop loop report` to record local loop contracts, guarded runner passes, pre-flight decisions, token receipts, iteration decisions, and stop reasons. Use `loop run` only when the loop contract has an explicit runner command.
