...
- Run `agentloop context pack --for codex --goal continue --redact-paths` before broad file reads or long-session continuation. Use `agentloop context show <handle>` to expand local source truth only when needed.
