...
- Run `agentloop start --for generic --goal implement --redact-paths` before broad file reads or long-session continuation. Use `agentloop context show <handle>` to expand local source truth only when needed.
