...
function printReport(report: HarnessUpgradeReport) {
...
export function upgradeHarnessCommand() {
  return new Command('upgrade-harness')
    ...
    .action(async (options: { dryRun?: boolean; json?: boolean; redactPaths?: boolean }) => {
      const workspace = await loadWorkspaceForJsonCommand(process.cwd(), options.json === true);
      if (!workspace) return;
      const rawReport = await inspectHarnessUpgrade({ cwd: workspace.cwd, dryRun: options.dryRun });
