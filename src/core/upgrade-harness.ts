...
const TOPICS: TopicDefinition[] = [
  ...
  {
    id: 'loop-control',
    needles: ['agentloop ready', 'agentloop loop', 'agentloop loop run', 'agentloop loop scorecard'],
    match: 'all',
  },
...
const TOPIC_SUGGESTIONS: Record<HarnessTopicId, { title: string; copyMarkdown: string }> = {
  ...
  'loop-control': {
    ...
    copyMarkdown:
      '- Use `agentloop ready` for a read-only readiness gate, and use `agentloop loop create`, `agentloop loop tick`, `agentloop loop run`, `agentloop loop scorecard`, `agentloop loop status`, and `agentloop loop report` to record local loop contracts, guarded runner passes, pre-flight decisions, token receipts, and stop reasons. Use `loop run` only when the loop contract has an explicit runner command.',
  },
  ship: {
