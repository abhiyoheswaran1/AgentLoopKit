...
describe('release smoke script helpers', () => {
  ...
  test('accepts README redaction guidance with every shareable redaction command', () => {
    const readme = [
      'Use `--redact-paths` with `doctor`, `status`, `next`, `review-context`, `check-gates`, `ship`, `prepare-pr`, or `maintainer-check` before pasting output into a public issue, PR, or CI log.',
    ].join('\n');
