...
If `task doctor` reports `post-verification-gate-in-verification-commands`, move the listed command from `Verification Commands` to `Post-Verification Gates`. Verification commands should be runnable before a fresh AgentLoop report exists; post-verification gates are for commands such as `npm run dogfood:strict`, `agentloop check-gates --strict`, or `agentloop release-check --strict`.
