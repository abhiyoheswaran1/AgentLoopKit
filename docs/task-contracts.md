...
Use `agentloop task doctor --json` to find task contracts that still need status cleanup, archiving, or post-verification gate cleanup. The command is read-only and ignores files already moved into `.agentloop/tasks/archive/`.
If `task doctor` reports `post-verification-gate-in-verification-commands`, move the listed command from `Verification Commands` to `Post-Verification Gates`. Verification commands should be runnable before a fresh AgentLoop report exists; post-verification gates are for commands such as `npm run dogfood:strict`, `agentloop ship`, `agentloop prepare-pr`, `agentloop check-gates`, `agentloop maintainer-check`, or `agentloop release-check --strict`.
