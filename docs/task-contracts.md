...
Use `agentloop create-task --json` when an agent needs the created task path and Markdown content without parsing the human success line. If you pass `--out`, the path must be a Markdown file inside the configured task directory. Invalid `--out` paths return parseable JSON errors with `requestedOut`, `tasksDir`, and `reason`.
