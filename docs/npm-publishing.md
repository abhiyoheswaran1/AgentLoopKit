...
## Current `0.7.0` Publishing State
...
Publish `0.7.0` to npm only after npm trusted publishing is configured or local browser authentication succeeds.
...
For `v0.7.0`, the expected successful result is latest `0.7.0` and a versions list containing `0.7.0`.
