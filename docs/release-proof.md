...
If a repository does not publish an MCP server, `release-proof` still checks npm, GitHub Releases, GitHub Marketplace, and GHCR. The MCP Registry channel reports a warning instead of stopping the command. Use `--strict` when that warning should fail a release gate.
...
agentloop release-proof \
  ...
  --github-marketplace-json github-marketplace.json \
...
`release-proof` does not publish packages, create tags, create GitHub releases, publish GitHub Marketplace listings, read npm tokens, read GitHub tokens, read `.env` files, upload files, post comments, or change package metadata.

In live mode it queries public metadata with a timeout. GitHub Marketplace proof checks the public listing page only. GHCR may return a registry challenge first; AgentLoopKit requests the anonymous public bearer token needed to read the public tag list. It does not read local Docker, GitHub, or npm credentials.
