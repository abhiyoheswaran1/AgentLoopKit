...
```json
{
  "mcpServers": {
    "agentloopkit": {
      "command": "npx",
      "args": ["--yes", "agentloopkit@0.26.4", "mcp-server"]
    }
  }
}
```
...
The MCP server is read-only in `0.26.4`.
...
The MCP Registry publish workflow runs only after the npm publish workflow succeeds for the same version, or when a maintainer starts it manually. It verifies that `agentloopkit@0.26.4` exists on npm before submitting server metadata.
