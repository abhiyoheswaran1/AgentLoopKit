...
`init` writes files into the current directory. The output shows the target folder, detected project type, package manager, Git status, configured commands, and file counts. Use `--dry-run` first when you want to see the same plan without writing files.
