...
| GitHub Action   | Live repo action      | `uses: abhiyoheswaran1/AgentLoopKit@v0.26.5`                                              |
| Docker / GHCR   | Live image            | `docker run --rm -v "$PWD:/workspace" ghcr.io/abhiyoheswaran1/agentloopkit:0.26.5 doctor` |
| MCP Registry    | Published metadata    | `npx --yes agentloopkit@0.26.5 mcp-server`                                                |
...
```yaml
- uses: abhiyoheswaran1/AgentLoopKit@v0.26.5
  with:
    command: check-gates --strict
    agentloopkit-version: 0.26.5
```
...
```bash
docker run --rm -v "$PWD:/workspace" ghcr.io/abhiyoheswaran1/agentloopkit:0.26.5 doctor
```
