...
| GitHub Action   | Live repo action      | `uses: abhiyoheswaran1/AgentLoopKit@v0.27.0`                                              |
| Docker / GHCR   | Live image            | `docker run --rm -v "$PWD:/workspace" ghcr.io/abhiyoheswaran1/agentloopkit:0.27.0 doctor` |
| MCP Registry    | Published metadata    | `npx --yes agentloopkit@0.27.0 mcp-server`                                                |
...
```bash
docker run --rm -v "$PWD:/workspace" ghcr.io/abhiyoheswaran1/agentloopkit:0.27.0 doctor
```
