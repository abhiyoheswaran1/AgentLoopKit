...
| Accumulate repeated task flags      | Lina, Maya             | Repeating `create-task --constraint` keeps only the last value.                   | Fix CLI option parsers so repeated values append instead of replace.                           | P2       | S      | medium          | low    | later    | Found while dogfooding Cycle 16.                           |
