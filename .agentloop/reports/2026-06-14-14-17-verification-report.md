# Verification Report

- Timestamp: `2026-06-14T12:17:21.100Z`
- Repo: `AgentLoopKit`
- Git branch: `main`
- Git commit: `c226ef6`
- Working tree: `dirty`
- Overall status: pass


## Task Context
- Path: `.agentloop/tasks/2026-06-14-release-agentloopkit-0-32-0.md`
- Title: `Release AgentLoopKit 0.32.0`
- Task type: `release`
- Status: `proposed`





## Commands Run
### custom: `node scripts/prepublish-check.mjs`

- Exit code: 0
- Status: pass


```text
Prepublish metadata check passed.
```

### custom: `npm run lint`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 lint
> eslint .

```

### custom: `npm run typecheck`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 typecheck
> tsc --noEmit

```

### custom: `npm test`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 test
> vitest run


 RUN  v4.1.8 /Users/abhyoh/local dev folder/Apps/AgentLoopKit


 Test Files  58 passed (58)
      Tests  546 passed (546)
   Start at  14:17:36
   Duration  261.83s (transform 476ms, setup 0ms, import 2.85s, tests 2595.33s, environment 3ms)

```

### custom: `npm run build`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 build
> tsup && node scripts/copy-assets.mjs

CLI Building entry: {"cli/index":"src/cli/index.ts"}
CLI Using tsconfig: tsconfig.json
CLI tsup v8.5.1
CLI Using tsup config: /Users/abhyoh/local dev folder/Apps/AgentLoopKit/tsup.config.ts
CLI Target: node20
CLI Cleaning output folder
ESM Build start
ESM dist/cli/index.js     375.46 KB
ESM dist/cli/index.js.map 713.60 KB
ESM ⚡️ Build success in 32ms
DTS Build start
DTS ⚡️ Build success in 920ms
DTS dist/cli/index.d.ts 13.00 B
```

### custom: `npm run check:public-docs`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 check:public-docs
> node scripts/public-docs-hygiene.mjs

# AgentLoopKit Public Docs Hygiene
Version: 0.32.0
Public docs checked: 71
Repo harness files checked: 2
Public docs hygiene passed.
```

### custom: `npm run check:links`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 check:links
> tsx scripts/check-markdown-links.ts

Markdown links OK (1819 file(s) checked).
```

### custom: `npm run smoke:release`

- Exit code: 0
- Status: pass


```text

> agentloopkit@0.32.0 smoke:release
> node scripts/smoke-packed-release.mjs

Release smoke for agentloopkit@0.32.0
Packed /var/folders/tc/vn86wd6d7v70rn_pxbv3h2k40000gn/T/agentloopkit-release-smoke-Yk3T8e/pack/agentloopkit-0.32.0.tgz
README has no stale exact version pins.
Public docs hygiene passed for 71 public docs and 2 harness files.
Packed binary version smoke passed.
Packed init smoke passed.
Packed local-only init smoke passed.
Packed create-task path guard smoke passed.
Packed verify task path guard smoke passed.
Packed init symlink guard smoke passed.
Packed task archive symlink guard smoke passed.
Packed home-directory dry-run guard smoke passed.
Release smoke passed.
```

## Not Run
- test
- lint
- typecheck
- build

## Recommended Next Actions
- Review the diff and prepare a handoff summary.
