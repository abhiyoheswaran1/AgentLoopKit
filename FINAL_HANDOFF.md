...
- repo-native Start preflight with `agentloop start`, decisive preflight states, source handles, risk summary, verification freshness, next safe command, and context-budget impact
...
- Neutral `agentloop ready` context-budget receipts for idle repos with no changed-file context.
- Clearer release-channel docs for repository-ref GitHub Action usage while GitHub Marketplace listing proof remains pending.
...
- Guarded local loop runner passes with `agentloop loop run`.
- Explicit `loop create --runner-command` configuration for local automation.
- Non-shell runner execution with blocked publish, destructive, and git state-changing command families.
- Runner evidence that records exit code, bounded output, changed files, token receipts, and normal loop stop decisions.
- README demo GIF and screenshot placement improvements for the current AgentLoopKit workflow.
- Updated CLI reference, loop-contract docs, generated harness guidance, and dogfood guidance.
...
- `agentloop context handles` for current source-handle inventory and expansion commands.
- Start usefulness proof with context avoided, broad files avoided, stale proof, scope drift, source handles, verification freshness, and next safe command.
- Doctor Agent Readiness Matrix for Start, context handles, broad-read avoidance, MCP guidance, and installed agent instructions.
- Compact Guard JSON for large dirty repos, with bounded evidence maps and source-handle guidance.
- Compact review-context, context-pack, MCP, and run-ledger payloads that point to local source handles instead of broad evidence dumps.
...
- `agentloop start` as the repo-native preflight for software agents.
- Decisive Start states for ready continuation, missing task, missing or stale verification, scope drift, review-ready work, blocked risk, and evidence-only sessions.
- Read-first source handles, risk summary, verification freshness, next safe command, and context-budget impact in Start output.
- `agentloop_start` in the read-only MCP surface.
- Generated agent guidance that asks Codex, Claude Code, Cursor, OpenCode, Gemini CLI, GitHub Copilot CLI, and generic agents to run `agentloop start` before broad repo reads.
- README and docs coverage for the Start preflight workflow plus the regenerated terminal demo GIF.
...
- core artifact path primitives moved into a leaf module to remove the `artifacts`/`runs`/`task-state` cycle.
- clearer direct AgentFlight placeholder recovery guidance.
- a compact first-use loop in README and getting-started docs.
- `prepublishOnly` release-guard trust-boundary documentation.
- a real-repo trial checklist for local policy-pack and GitHub metadata trials.
- public-doc hygiene coverage that protects real-repo trial guidance from marketing, remote-service, telemetry, token, and scoring drift.
...
- explicit `author` metadata in the root composite GitHub Action.
- Action metadata formatting aligned with the Marketplace-published ProjScan Action.
- a Marketplace-focused patch release so GitHub re-evaluates the released Action metadata.

## 0.35.1 release summary

Included:

- GitHub Marketplace proof in `agentloop release-proof`, including `--only github-marketplace`.
- captured `--github-marketplace-json` support for deterministic release-proof fixtures.
- release docs and checklists that require the GitHub release UI Marketplace checkbox and category before claiming Marketplace availability.
...
- GitHub release `v1.1.0` is public.
- npm latest is `agentloopkit@1.1.0`.
- GHCR tags include `latest`, `1.1`, and `1.1.0`.
- MCP Registry lists `1.1.0` as latest.
