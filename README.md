...
`init` writes files into the current directory. The output shows the target folder, detected project type, package manager, configured commands, and file counts. Do not run it from `~` unless you intend to configure your home directory. `--dry-run` previews the same plan and writes nothing.
