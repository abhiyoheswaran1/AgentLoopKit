...
Use `--redact-paths` with `status`, `next`, `review-context`, `check-gates`, `ship`, or `prepare-pr` before pasting output into a public issue, PR, or CI log. That mode replaces the absolute Git root with `[git-root]`.
